...
// Import AndroidImportance explicitly
const {AndroidImportance} = messaging;
  ...
  useEffect(() => {
    const requestNotificationPermission = async () => {
      try {
        ...
        if (!enabled) {
          Toasts('Error', 'Notification permission denied.', 'error', 4000);
        }
      } catch (error) {
    ...
    if (Platform.OS === 'android') {
      ...
      const channelId = 'zab_find_notifications';
      messaging().createLocalNotificationChannel({
        id: channelId,
        name: 'Zab Find Notifications',
        description: 'Notifications for lost and found items',
        importance: AndroidImportance?.HIGH || 4, // Fallback to 4 if HIGH is undefined
        vibrate: true,
        soundName: 'default',
        lights: true,
        lightColor: '#FFFFFF',
