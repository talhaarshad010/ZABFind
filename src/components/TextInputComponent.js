  ...
  (
    {
      ...
      onSubmitEditing,
      error = false,
      secureTextEntry = false, // Add secureTextEntry as a prop
      ...rest
    },
    ref,
  ) => {
    const [isPasswordVisible, setPasswordVisible] = useState(!secureTextEntry); // Initialize based on secureTextEntry prop
        ...
        <View
          ...
          <View
            ...
            <TextInput
              ref={ref}
              allowFontScaling={false}
              secureTextEntry={secureTextEntry && !isPasswordVisible} // Use prop and state
              cursorColor={Colors.black}
              placeholder={placeholder}
              onChangeText={onChangeText}
              value={value}
...
const styles = StyleSheet.create({
  ...
  textInput: {
    flex: 1, // Fix: Remove fixed width to make it flexible
    color: Colors.black,
    fontSize: responsiveFontSize(1.7),
  },
  fieldName: {
