...
const CustomDrawerContent = props => {
  ...
  useEffect(() => {
    console.log('lost items in custom drawer:', {
      data,
      error,
      isLoading,
      isFetching,
  ...
  // Dynamically set infoData based on user role
  const infoData = isAdmin
    ? [
        {label: 'Full Name', value: user?.fullName || 'N/A'},
        {label: 'Admin ID', value: user?.adminId || 'N/A'},
    ...
    : [
        ...
        {label: 'Phone', value: user?.phoneNumber || 'N/A'},
        {
          label: 'Address',
          value:
            user?.address?.street + ', ' + (user?.address?.city || 'N/A') ||
            'N/A',
        },
    ...
    <WrapperContainer>
      <DrawerContentScrollView
        ...
        <View style={styles.container}>
          ...
          {/* Member Since */}
          <MyText
            text={`Member since ${new Date(user?.updatedAt).getFullYear()}`}
